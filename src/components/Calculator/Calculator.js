...
class Calculator extends Component {
    ...
    onButtonPress = (event) => {
        ...
        if (pressedButtonValue === 'C') {
            ...
            /* If the user has pressed a button with a value between 0 and 9 or the period,
            add that value to the equation */
            equation += pressedButtonValue;
        } else if (['+', '-', '*', '/', '%'].indexOf(pressedButtonValue) !== -1) {
            /* If the user has pressed a button with the value of an operator, add the operator
            to the equation with a space on either side */
            equation += ' ' + pressedButtonValue + ' ';
        } else if (pressedButtonValue === '=') {
            /* If the user hits the enter button, calculate stuff */
            /* Try catch blocks - try to run the code in the try block. If it fails and throws an error,
            deal with it in the catch block */
            try {
                const evalResult = eval(equation);
                const result = Number.isInteger(eval) ? evalResult : evalResult.toFixed(2);
                this.setState({ result }); 
            } catch (error) {
                /* If an error occurs, print it to the console and alert the user */
                console.log(error);
                alert('An error ocurred. Check to make sure your equation is correct');
            }
        }  else {
